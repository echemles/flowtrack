...
export function LiveTrackingPanel({ data }: { data: Live }) {
  ...
  // Header (ref + mode/status), route+carrier, progress — kept compact, wraps to two rows on mobile.
  const Header = (
    <div className="border border-brand-rule bg-brand-paper p-3 sm:p-4">
      <div className="flex flex-wrap items-center gap-2 text-[14px] font-medium text-brand-navy">
        ...
        {daysRemaining != null ? (
          <span className="ft-micro ml-auto text-brand-red">● {daysRemaining}d remaining</span>
        ) : null}
      </div>
      <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[13px] text-brand-navy/65">
        <span>
        ...
        {s.carrier ? (
          <span className="ft-micro text-brand-navy/55">· {s.carrier}</span>
        ) : null}
      </div>
      ...
      <div className="ft-micro mt-2 flex items-center justify-between gap-2 text-brand-navy/55">
        <span className="whitespace-nowrap">ATD {formatDateLong(s.atd)}</span>
        <span
          className={clsx(
            'whitespace-nowrap',
            percent >= 90 ? 'text-brand-navy' : 'text-brand-red',
          )}
        >
          {percent >= 100 ? 'Delivered' : `${percent}% complete`}
        </span>
        <span className="whitespace-nowrap">ETA {formatDateLong(s.eta_agi)}</span>
      </div>
    </div>
  );
  ...
  const ShipmentInfo = (
    <div className="border border-brand-rule bg-brand-paper p-4">
      ...
      <dl className="grid grid-cols-1 gap-x-6 text-[14px] sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
      ...
      <a
        href={`/app/shipments/${s.ref}`}
        className="mt-4 inline-flex w-full min-h-[44px] items-center justify-center border border-brand-navy bg-brand-navy px-4 py-2 text-[13px] font-medium uppercase tracking-wide text-brand-paper transition-colors hover:bg-brand-navy/90"
      >
        Open full detail →
      </a>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {/* <lg : stacked map -> info -> milestones.
          lg+ : map (3fr) left, info+milestones (2fr) right. min-w-0 prevents the map SVG from pushing the cell wider than its share. */}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[3fr_2fr]">
        <div className="min-w-0 lg:order-1">{Map}</div>
